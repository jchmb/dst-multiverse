...
local function CalcSanityAura(inst, observer)
    return (inst.components.werebeast ~= nil and inst.components.werebeast:IsInWereState() and -TUNING.SANITYAURA_LARGE)
        ...
        or ((inst.components.follower == nil or inst.components.follower.leader ~= observer) and inst:GetIsWet() and -TUNING.SANITYAURA_SMALL) 
        or 0
end
...
local function OnEat(inst, food)
    if food.components.edible ~= nil then
        if food.components.edible.foodtype == FOODTYPE.VEGGIE then
            SpawnPrefab("wetgoop").Transform:SetPosition(inst.Transform:GetWorldPosition())
...
return Prefab("pigman_yellow", normal, assets, prefabs)
