...
local SlimeyPigBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function SlimeyPigBrain:OnStart()
    --print(self.inst, "PigBrain:OnStart")
    local day = WhileNode( function() return TheWorld.state.isday end, "IsDay",
        PriorityNode{
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_FIND_MEAT",
                DoAction(self.inst, FindFoodAction )),
            IfNode(function() return StartChoppingCondition(self.inst) end, "chop", 
                WhileNode(function() return KeepChoppingAction(self.inst) end, "keep chopping",
                    LoopNode{ 
                        ChattyNode(self.inst, "PIG_SLIMEY_TALK_HELP_CHOP_WOOD",
                            DoAction(self.inst, FindTreeToChopAction ))})),
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_FOLLOWWILSON",
            ...
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_RUNAWAY_WILSON",
                RunAway(self.inst, "player", START_RUN_DIST, STOP_RUN_DIST)),
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_LOOKATWILSON",
                FaceEntity(self.inst, GetFaceTargetFn, KeepFaceTargetFn)),
    ...
    local night = WhileNode( function() return not TheWorld.state.isday end, "IsNight",
        PriorityNode{
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_RUN_FROM_SPIDER",
            ...
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_GO_HOME",
            ...
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_FIND_LIGHT",
                FindLight(self.inst, SEE_LIGHT_DIST, SafeLightDist)),
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_PANIC",
                Panic(self.inst)),
        }, 1)
    ...
    local root =
        PriorityNode(
        {
            WhileNode( function() return self.inst.components.hauntable and self.inst.components.hauntable.panic end, "PanicHaunted",
                ChattyNode(self.inst, "PIG_SLIMEY_TALK_PANICHAUNT",
                    Panic(self.inst))),
            WhileNode(function() return self.inst.components.health.takingfiredamage end, "OnFire",
                ChattyNode(self.inst, "PIG_SLIMEY_TALK_PANICFIRE",
                    Panic(self.inst))),
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_FIGHT",
                WhileNode( function() return self.inst.components.combat.target == nil or not self.inst.components.combat:InCooldown() end, "AttackMomentarily",
                    ChaseAndAttack(self.inst, MAX_CHASE_TIME, MAX_CHASE_DIST) )),
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_RESCUE",
            ...
            WhileNode(function() return IsHomeOnFire(self.inst) end, "OnFire",
                ChattyNode(self.inst, "PIG_SLIMEY_TALK_PANICHOUSEFIRE",
                    Panic(self.inst))),
            ...
            ChattyNode(self.inst, "PIG_SLIMEY_TALK_ATTEMPT_TRADE",
