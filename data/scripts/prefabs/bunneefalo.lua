local assets =
{
    ...
    Asset("ANIM", "anim/bunneefalo_build.zip"),
    Asset("ANIM", "anim/bunneefalo_shaved_build.zip"),
    Asset("ANIM", "anim/bunneefalo_baby_build.zip"),
...
local function ClearBuildOverrides(inst, animstate)
    if animstate ~= inst.AnimState then
        animstate:ClearOverrideBuild("bunneefalo_build")
    end
...
local function ApplyBuildOverrides(inst, animstate)
    ...
    local basebuild = (inst:HasTag("baby") and "bunneefalo_baby_build")
            or (inst.components.beard.bits == 0 and "bunneefalo_shaved_build")
            or (inst.components.domesticatable:IsDomesticated() and "beefalo_domesticated") -- TODO
            or "bunneefalo_build"
...
local function bunneefalo()
    ...
    inst.AnimState:SetBuild("bunneefalo_build")
    inst.AnimState:PlayAnimation("idle_loop", true)
    inst.AnimState:Hide("HEAT")

    inst:AddTag("bunneefalo")
    inst:AddTag("animal")
...
return Prefab("bunneefalo", bunneefalo, assets, prefabs)
