local assets =
{
    ...
    Asset("ANIM", "anim/pig_cyborg_build.zip"),
...
local function SetNormalPig(inst)
    ...
    inst.components.combat:SetDefaultDamage(40)--TUNING.PIG_DAMAGE)
    inst.components.combat:SetAttackPeriod(TUNING.PIG_ATTACK_PERIOD)
    inst.components.combat:SetKeepTargetFunction(NormalKeepTargetFn)
    inst.components.locomotor.runspeed = 4--TUNING.PIG_RUN_SPEED
    inst.components.locomotor.walkspeed = 2--TUNING.PIG_WALK_SPEED
...
local function common()
    ...
    inst.components.werebeast:SetTriggerLimit(999)
...
local function normal()
    ...
    inst.build = "pig_cyborg_build"
    inst.AnimState:SetBuild(inst.build)
    SetNormalPig(inst)
    return inst
end

return Prefab("pigman_cyborg", normal, assets, prefabs)
