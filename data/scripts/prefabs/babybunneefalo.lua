...
local function OnAttacked(inst, data)
    ...
    inst.components.combat:ShareTarget(data.attacker, 30, function(dude)
        return dude:HasTag("bunneefalo") and not dude:HasTag("player") and not dude.components.health:IsDead()
    end, 5)
end
...
local function FollowGrownBeefalo(inst)
    ...
    {"bunneefalo", "herdmember"}, -- only follow herd beefalo (i.e. nondomesticated)
    {"baby"}
    )
...
local function SetFullyGrown(inst)
    local grown = SpawnPrefab("bunneefalo")
...
local function fn()
    ...
    inst.AnimState:SetBuild("bunneefalo_baby_build")
...
return Prefab("babybunneefalo", fn, assets, prefabs)
