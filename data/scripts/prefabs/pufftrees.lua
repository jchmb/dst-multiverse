local assets =
{
    ...
    Asset("ANIM", "anim/pufftree_build.zip"), --poison leaves build
    Asset("ANIM", "anim/pufftree_trunk_build.zip"), --poison leaves build
    Asset("ANIM", "anim/pufftree_leaf_fx.zip"), --poison leaves build
...
{
    normal = { --Green
        leavesbuild="pufftree_build",
        prefab_name="pufftree",
        normal_loot = {"log"},
        short_loot = {},
        tall_loot = {"log", "log", "pufftree_nut"},
        drop_acorns=true,
        fx="green_leaves",
...
local function GrowLeavesFn(inst, monster, monsterout)
    ...
    if inst.leaf_state == "barren" then
        ...
        inst.AnimState:OverrideSymbol("mouseover", "pufftree_trunk_build", "toggle_mouseover")
    end
...
local function chop_down_tree(inst, chopper)
    ...
    if he_right then
        ...
        if inst.components.growable ~= nil and inst.components.growable.stage == 3 and inst.leaf_state == "colorful" then
            inst.components.lootdropper:SpawnLootPrefab("pufftree_nut", pt - TheCamera:GetRightVec())
        end
...
local function onburntchanges(inst)
    ...
    if GetBuild(inst).drop_acorns then
        inst.components.lootdropper:AddChanceLoot("pufftree_nut", .1)
    end
...
local function _OnBurnt2(inst)
    if inst.monster then
        inst.monster = false
        -- inst:RemoveComponent("pufftreeupdater")
...
local function onignite(inst)
    ...
    -- if inst.components.pufftreeupdater ~= nil then
    --     inst.components.pufftreeupdater:SpawnIgniteWave()
    -- end
end
...
local function makefn(build, stage, data)
    return function()
        ...
        inst.MiniMapEntity:SetIcon("minimap_pufftree.tex")
        inst.MiniMapEntity:SetPriority(-1)

        inst:AddTag("tree")
        inst:AddTag("birchnut")
        inst:AddTag("cattoyairborne")
        inst:AddTag("pufftree")
        inst:AddTag("shelter")

        inst.build = build
        inst.AnimState:SetBank("tree_leaf")
        inst.AnimState:SetBuild("pufftree_trunk_build")
        ...
        -- inst.components.plantregrowth:SetProduct("pufftree_sapling")
        -- inst.components.plantregrowth:SetSearchTag("pufftree")
...
return tree("pufftree", "normal", 0),
        tree("pufftree_normal", "normal", 2),
        tree("pufftree_tall", "normal", 3),
        tree("pufftree_short", "normal", 1),
        tree("pufftree_burnt", "normal", 0, "burnt"),
        tree("pufftree_stump", "normal", 0, "stump")
