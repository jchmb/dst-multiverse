local assets =
{
    Asset("ANIM", "anim/pig_king_cyborg.zip"),
...
local function ontradeforgold(inst, item)
    ...
    local value = math.max(
        1,
        math.floor(item.components.tradable.goldvalue / 2)
    )
    for k = 1, value do
        ThrowReward(inst, "ironnugget")
    end
end
...
local function fn()
    ...
    inst:AddTag("king")
    inst.AnimState:SetBank("Pig_King_Cyborg_Cyborg")
    inst.AnimState:SetBuild("Pig_King_Cyborg")
...
return Prefab("pigking_cyborg", fn, assets, prefabs)
