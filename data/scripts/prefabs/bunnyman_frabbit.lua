local assets =
{
	...
	Asset("ANIM", "anim/bunnyman_frabbit_build.zip"),
...
local brain = require("brains/frabbitbrain")
...
local function NormalRetargetFn(inst)
	return FindEntity(inst, TUNING.PIG_TARGET_DIST,
		function(guy)
			return inst.components.combat:CanTarget(guy) and false
		end,
...
local function OnKilled(inst)
    inst.components.timer:StartTimer("regen_bunnyman_frabbit", TUNING.DRAGONFLY_RESPAWN_TIME / 4)
end

local function GenerateNewBunnymanFrabbit(inst)
    inst.components.childspawner:AddChildrenInside(1)
    inst.components.childspawner:StartSpawning()
end

local function ontimerdone(inst, data)
    if data.name == "regen_bunnyman_frabbit" then
        GenerateNewBunnymanFrabbit(inst)
    end
end
...
local function spawner_fn()
    ...
    inst.components.childspawner.childname = "bunnyman_frabbit"
...
local function fn()
	...
	inst.AnimState:SetBuild("bunnyman_frabbit_build")
	...
	-- inst.AnimState:SetClientsideBuildOverride("insane", "bunnyman_frabbit_build", "manrabbit_beard_build")
	...
	inst:DoTaskInTime(1, function()
		...
		inst.statue = FindEntity(inst, 20, function(ent) return ent.prefab == "statue_snook" end)
	end)

	return inst
end

return Prefab("bunnyman_frabbit", fn, assets, prefabs),
	Prefab("bunnyman_frabbit_spawner", spawner_fn, nil, prefabs)
